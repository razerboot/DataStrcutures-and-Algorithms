...
def cal_counters(arr, primes, counters):
    ...
    for i in xrange(n):
        if arr[i] != 1:
            counters[i + 2][arr[i]] += 1


def cal_prefix_counters(arr, prefix_counters, counters, n, mod):
    '''we preprocess prefix counters'''

    for i in xrange(n):
        r = i % mod
        if r == 0:
            prefix_counters[i] = counters[arr[i]]
        else:
            prefix_counters[i] = prefix_counters[i - 1] + counters[arr[i]]


def ans_queires(arr, counters, prefix_counters, q, mod):
    for a0 in xrange(q):
        l = randrange(n)
        r = randrange(l, n)
        ...
        if fr < cl:
            ...
            if r1 != 0:
                c = prefix_counters[cl * mod - 1] - prefix_counters[l - 1]
            else:
                c = prefix_counters[cl * mod - 1]
            ...
            for k in xrange((cl + 1) * mod - 1, fr * mod, mod):
                for key in prefix_counters[k]:
                    if x <= key <= y:
                        count += c[key]
...
#q = input()
arr1 = [i for i in xrange(2, max(arr) + 1)]
t1 = time()
primes = seive(10 ** 3)
#print time() - t1
...
t1 = time()
cal_counters(arr1, primes, counters)
cal_prefix_counters(arr, prefix_counters, counters, n, mod)

print time() - t1
#print counters

t1 = time()
ans_queires(arr, counters, prefix_counters, q, mod)
print time() - t1
